...
#define CM_ORDER_LOCAL        90   
*/
void sendData(const Data *data)
{
  
    //if exist insert insert many
    ...
    int fd = open("client_to_master",O_WRONLY);
     myassert(fd != -1 , "toto");
     int ret = write(fd,&(data->order),sizeof(int));
     myassert(ret != -1,"toto");

    if(data->order == CM_ORDER_INSERT || data->order == CM_ORDER_EXIST)
    {
         int ret2 = write(fd,&(data->elt),sizeof(float));
         myassert(ret2 != -1,"toto");
    } 
    if (data->order == CM_ORDER_INSERT_MANY)
    {
         int ret2 = write(fd,&(data->nb),sizeof(int));
         myassert(ret2 != -1,"toto");
         int ret3 = write(fd,&(data->min),sizeof(int));
         myassert(ret3 != -1,"toto");
         int ret4 = write(fd,&(data->max),sizeof(int));
         myassert(ret4 != -1,"toto");
    }
    close(fd);
...
{
    /*int fd2 = open("master_to_client",O_RDONLY);
    myassert(fd2 != -1 ,"toto");
    read(fd2, &(data->order), sizeof(int));*/
    ...
    myassert(fd2 != -1 ,"toto");
    float a;
    int ret = read(fd2,&a,sizeof(float));
     myassert(ret != -1,"toto");
